...
func (cs *commentSrv) Add(token interface{}, newComment comments.Core) (comments.Core, error) {
	...
	if userID <= 0 {
		return comments.Core{}, errors.New("user tidak ditemukan")
	}
	...
	// 		log.Println(err)
	// // 	}
	// 	return comments.Core{}, errors.New("input buku tidak sesuai dengan arahan")
	// }
...
func (cs *commentSrv) GetComment() ([]comments.Core, error) {
	res, err := cs.data.GetComment()
	if err != nil {
		msg := ""
		if strings.Contains(err.Error(), "not found") {
