...
func TestAdd(t *testing.T) {
	...
	t.Run("Success Add", func(t *testing.T) {
		...
		id.Valid = true
		res, err := srv.Add(id, multipart.FileHeader{}, inputData)
		assert.Nil(t, err)
...
func TestGetPostDetail(t *testing.T) {
	...
	t.Run("data tidak ditemukan", func(t *testing.T) {
		...
		repo.On("GetPostDetail", 5).Return(posts.Core{}, errors.New("data not found")).Once()

		srv := New(repo)

		res, err := srv.GetPostDetail(5)
		assert.NotNil(t, err)
	...
	t.Run("masalah di server", func(t *testing.T) {
		...
		repo.On("GetPostDetail", 1).Return(posts.Core{}, errors.New("terdapat masalah pada server")).Once()
