...
// Update provides a mock function with given fields: token, file, updateData
func (_m *UserService) Update(token interface{}, file multipart.FileHeader, updateData users.Core) (users.Core, error) {
	ret := _m.Called(token, file, updateData)

	var r0 users.Core
	if rf, ok := ret.Get(0).(func(interface{}, multipart.FileHeader, users.Core) users.Core); ok {
		r0 = rf(token, file, updateData)
	} else {
		r0 = ret.Get(0).(users.Core)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(interface{}, multipart.FileHeader, users.Core) error); ok {
		r1 = rf(token, file, updateData)
	} else {
		r1 = ret.Error(1)
	}
